...
export class SendSmsAction {
  ...
  public static async documentHelperMessage(
    reminder: Boolean,
    hi: HelpersBase,
    type: string
  ) {
    let h = await hi.getHelper()
    if (reminder) h.reminderSmsDate = new Date()
    else h.smsDate = new Date()
    await h.save()
  }

  static async generateMessage(
    helperIn: HelpersBase,
    origin: string,
    reminder: Boolean,
    senderName: string,
    then: (
    ...
    if (helper) {
      ...
      message = SendSmsAction.getMessage(
        message,
  ...
  static getMessage(
    ...
    return template
      .replace('!מתנדב!', courier)
      .replace('!משנע!', courier)
      .replace('!VOLUNTEER!', courier)
      .replace('!משפחה!', family)
...
export class SendSmsUtils {
  ...
  async sendSms(
    ...
    if (settings.bulkSmsEnabled) {
      if (settings.smsVirtualPhoneNumber) from = settings.smsVirtualPhoneNumber
      ...
      try {
        ...
        if (settings.forWho.args.internationalPrefixForSmsAndAws) {
          if (SendSmsUtils.twilioSendSms) {
            let r = await SendSmsUtils.twilioSendSms(
              internationalPhone,
              message,
              settings.forWho
            )
